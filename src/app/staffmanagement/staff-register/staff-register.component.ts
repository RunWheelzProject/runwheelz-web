...
export class StaffRegisterComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.staffRegister.invalid) {
      ...
      this.subscription = this.registrationService.sendOTP('/api/auth/register/runwheelz/staff/sendOtp/', `?phoneNumber=${phonenumber}`).subscribe((data: any) => {
        this.message = data;
        console.log(data.phoneNumber);

        localStorage.setItem('staff_send_otp_response', JSON.stringify(data));
        this.router.navigate(['/verifynewstaff']);
